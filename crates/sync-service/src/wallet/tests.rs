...
use super::pending_output_poi::{
    pending_output_poi_observation_state_updates, pending_output_poi_submit_identity,
...
use super::{
    ...
    WalletHandle, WalletLiveMetadataFlush, WalletObservationPublisher, WalletPendingOverlay,
    WalletPendingSpent, WalletPersistState, WalletPoiRefreshSelection, WalletPoiRuntime,
    WalletPrivateApplyClient, WalletPrivateMutationAuthority, WalletPrivatePoiClients,
    WalletProgressPersist, WalletProgressPrivateEffects, WalletViewState,
    apply_owned_poi_private_delta_on_actor,
    apply_owned_poi_private_delta_on_actor_with_active_lists,
    apply_wallet_delta_to_vec_with_outcome, default_active_poi_list_keys,
...
use crate::txid_cache::{TxidPublicCacheKey, seed_verified_artifact_bound_for_test};
use crate::types::{
...
use local_db::{
    DbConfig, DbStore, OpaqueWalletPrivateRow, OutputPoiRecoveryAction, OutputPoiRecoveryRecord,
    OutputPoiRecoveryStatus, PendingOutputPoiContextRecord, PendingOutputPoiRole, WalletCacheKey,
    WalletMeta, WalletPrivateRecordKind, WalletSyncActorStateRecord,
};
...
fn sender_candidate_for_spent_input(
    wallet_id: WalletCacheKey,
    input: &WalletUtxo,
    spent_source: UtxoSource,
...
async fn sender_candidate_public_data_fence(
    db: &Arc<DbStore>,
    cfg: &WalletConfig,
    ...
    row_timestamp: u64,
) -> (ChainPublicDataPlane, SenderCandidatePublicDataFence) {
    let graph_response = serde_json::json!({
        "data": {
            "transactions": [{
                "id": "0x00",
                "blockNumber": candidate.source.block_number.to_string(),
                "blockTimestamp": row_timestamp.to_string(),
                ...
                "commitments": [hex::encode_prefixed(output_commitment)],
    ...
    .to_string()
    .into_bytes();
    let (endpoint, _requests) = spawn_http_response(graph_response).await;
    let public_data_plane = ChainPublicDataPlane::new(Arc::clone(db), Arc::new(AtomicU64::new(0)));
    let cache_key = DataPlanePublicTxidCacheKey::new(
        ChainScope {
    ...
    public_data_plane
        ...
        .await
        .expect("seed sender public TXID row");
    let rows = public_data_plane
        .txid_transactions_for_outer_hash(&cache_key, candidate.source.tx_hash)
        .expect("load sender public TXID row");
    let fence = SenderCandidatePublicDataFence::new(
        &public_data_plane,
        cache_key,
...
struct RecordingPendingOutputPoiSubmitter {
    ...
    next_remote_error: Mutex<Option<RecordingPendingOutputPoiRemoteError>>,
}

#[derive(Clone, Copy)]
enum RecordingPendingOutputPoiRemoteError {
    MissingRailgunTxid,
    Other,
}
...
impl RecordingPendingOutputPoiSubmitter {
    ...
    fn fail_next_missing_railgun_txid(&self) {
        *self.next_remote_error.lock().expect("remote error") =
            Some(RecordingPendingOutputPoiRemoteError::MissingRailgunTxid);
    }

    fn fail_next_other_remote_error(&self) {
        *self.next_remote_error.lock().expect("remote error") =
            Some(RecordingPendingOutputPoiRemoteError::Other);
    }

    fn take_error(&self) -> Option<PoiError> {
        if let Some(error) = self.next_remote_error.lock().expect("remote error").take() {
            let message = match error {
                RecordingPendingOutputPoiRemoteError::MissingRailgunTxid => {
                    "Could not find railgun txid"
                }
                RecordingPendingOutputPoiRemoteError::Other => "different remote failure",
            };
            return Some(PoiError::RpcRequest {
        ...
        if *fail_next {
            *fail_next = false;
            Some(PoiError::MerkleRootsRejected)
        } else {
            None
        }
    }
...
struct InvalidateOwnedSubstituteAfterSubmitter {
    handle: WalletHandle,
    list_key: FixedBytes<32>,
    calls: AtomicU64,
}
...
#[async_trait]
impl PendingOutputPoiSubmitter for InvalidateOwnedSubstituteAfterSubmitter {
    async fn submit_single_commitment_proofs(
        &self,
        _txid_version: &str,
        _chain_type: u8,
        ...
        unreachable!("recovered outgoing group uses transact submission")
    }
    ...
    async fn submit_transact_proof(
        ...
        self.calls.fetch_add(1, Ordering::AcqRel);
        self.handle.utxos.write().await[0]
            .utxo
            .poi
            .statuses
            .insert(self.list_key, PoiStatus::Missing);
        Ok(())
    }
}
...
fn local_status_reader_for_lists(
    blinded_commitment: FixedBytes<32>,
    list_keys: &[FixedBytes<32>],
    valid_list_keys: &[FixedBytes<32>],
) -> LocalPoiStatusReader {
    let caches = list_keys
        .iter()
        .copied()
        .map(|list_key| {
            ...
            if valid_list_keys.contains(&list_key) {
                cache
                    .apply_poi_leaves(0, &[U256::from_be_bytes(blinded_commitment.0)])
                    .expect("apply local POI leaf");
            }
            (list_key, cache)
        })
        .collect();
    LocalPoiStatusReader::new(WalletLocalPoiCaches::new_for_test(caches))
}

fn external_pending_output_record(
    cfg: &WalletConfig,
    byte: u8,
    list_key: FixedBytes<32>,
    ...
    let mut record =
        pending_output_record(cfg.chain.chain_id, FixedBytes::from([byte; 32]), list_key);
    record.wallet_id = cfg.cache_key.to_string();
    record.output_role = output_role;
    record.observation = Some(local_db::PendingOutputPoiObservation {
        output_tree: u64::from(byte),
        output_position: u64::from(byte) + 1,
        tx_hash: source(byte).tx_hash,
        block_number: u64::from(byte),
        block_timestamp: 1_700_000_000 + u64::from(byte),
    });
    record
...
fn recovered_outgoing_pending_group(
    cfg: &WalletConfig,
    list_key: FixedBytes<32>,
...
fn seed_recovered_outgoing_pending_group(
    ...
    for record in records {
        store
            ...
            .expect("store recovered outgoing context");
        ...
        recovery.source_tx_hash = record.observation.as_ref().expect("observation").tx_hash;
        store
            .put_output_poi_recovery(&recovery)
            .expect("store recovered outgoing recovery");
    }
}

fn owned_substitute_for_recovered_context(
    context: &PendingOutputPoiContextRecord,
    list_key: FixedBytes<32>,
) -> WalletUtxo {
    let observation = context.observation.as_ref().expect("group observation");
    let mut output = test_wallet_utxo(0xee);
    output.utxo.tree = u32::try_from(observation.output_tree).expect("test tree");
    output.utxo.position = observation.output_position;
    output.utxo.source = UtxoSource {
        tx_hash: observation.tx_hash,
        block_number: observation.block_number,
        block_timestamp: observation.block_timestamp,
    };
    output.utxo.poi.commitment = context.output_commitment;
    output.utxo.poi.npk = context.output_npk;
    output.utxo.poi.blinded_commitment = UtxoPoiMetadata::blinded_commitment_for(
        context.output_commitment,
        context.output_npk,
        output.utxo.tree,
        output.utxo.position,
    );
    output.utxo.poi.statuses = BTreeMap::from([(list_key, PoiStatus::Valid)]);
    output
}

struct SenderMaterializationTestFixture {
    cfg: WalletConfig,
    candidate: SenderTransactionCandidate,
    input: WalletUtxo,
    pending: Vec<PendingOutputPoiContextRecord>,
    recoveries: Vec<OutputPoiRecoveryRecord>,
    fence: SenderCandidatePublicDataFence,
}

fn sender_materialization_proof_outputs(
    pending: &[PendingOutputPoiContextRecord],
) -> Vec<FixedBytes<32>> {
    pending
        .iter()
        .map(|context| {
            ...
            .expect("materialization submit identity")
            .derived_blinded_commitment
        })
        .collect()
}

async fn sender_materialization_test_fixture(
    store: &Arc<DbStore>,
    wallet_name: &str,
    output_count: usize,
) -> SenderMaterializationTestFixture {
    let mut cfg = wallet_config(U256::from(42));
    cfg.cache_key = test_cache_key(wallet_name);
    ...
    store
        .put_opaque_wallet_private_row(
            ...
            &OpaqueWalletPrivateRow {
                ...
                payload: candidate.encode().expect("encode candidate"),
            },
        )
        .expect("store candidate");

    let mut pending = Vec::with_capacity(output_count);
    let mut recoveries = Vec::with_capacity(output_count);
    for output in &candidate.outputs {
        let note = output.note.clone().expect("qualified sender output");
        let utxo = Utxo::new(
            note,
            output.tree,
            output.position,
    ...
    let (_, fence) = sender_candidate_public_data_fence(
        store,
        &cfg,
        &candidate,
        &input,
        candidate.outputs[0].commitment,
        candidate.source.block_timestamp,
    )
    .await;
    SenderMaterializationTestFixture {
        cfg,
...
fn wallet_ppoi_workflow_status_classifies_only_observed_active_contexts() {
    ...
    let active_list = FixedBytes::from([0xc0; 32]);
    let inactive_list = FixedBytes::from([0xc1; 32]);

    let mut awaiting_submission =
        external_pending_output_record(&cfg, 0xc2, active_list, PendingOutputPoiRole::Recipient);
    awaiting_submission.submitted_poi_list_keys.clear();
    let mut awaiting_validation = external_pending_output_record(
        &cfg,
        0xc3,
        active_list,
    ...
    let mut unobserved =
        external_pending_output_record(&cfg, 0xc6, active_list, PendingOutputPoiRole::Recipient);
...
#[test]
fn wallet_ppoi_workflow_status_transitions_candidate_output_without_double_counting() {
    let root_dir = temp_db_root();
    let store = DbStore::open(DbConfig {
        root_dir: root_dir.clone(),
    })
    .expect("open db");
    let cfg = wallet_config(U256::ZERO);
    let active_list = FixedBytes::from([0xca; 32]);
    let input = test_wallet_utxo(0xcb);
    let mut candidate =
        sender_candidate_for_spent_input(cfg.cache_key.clone(), &input, source(0xcc));
    let owned_note = Note {
        token_hash: U256::from(7),
        value: U256::from(4),
        random: [0xce; 16],
        npk: Note::npk_for(cfg.scan_keys.master_public_key, [0xce; 16]),
    };
    candidate.outputs.push(SenderTransactionCandidateOutput {
        tree: 3,
        position: 5,
        commitment: FixedBytes::from(owned_note.commitment().to_be_bytes::<32>()),
        note: Some(owned_note),
    });
    candidate.outputs.push(SenderTransactionCandidateOutput {
        tree: 3,
        position: 6,
        commitment: FixedBytes::from([0xef; 32]),
        note: None,
    });
    candidate
        .validate()
        .expect("candidate with owned and unknown outputs");

    let awaiting_recovery = wallet_ppoi_workflow_status_after_mutations(
        &store,
        &cfg,
        &[active_list],
        0,
    ...
    let mut materialized = external_pending_output_record(
        &cfg,
        0xcd,
        active_list,
        PendingOutputPoiRole::RecoveredOutgoing,
    );
    materialized.output_commitment = candidate.outputs[0].commitment;
    let after_materialization = wallet_ppoi_workflow_status_after_mutations(
        &store,
        &cfg,
        &[active_list],
        0,
...
#[test]
fn wallet_ppoi_workflow_status_keeps_matching_valid_recovery_awaiting_reconciliation() {
    let root_dir = temp_db_root();
    let store = DbStore::open(DbConfig {
        root_dir: root_dir.clone(),
    })
...
struct RecordingCacheState {
    ...
    pending_output_list_calls: usize,
...
struct RecordingCacheStore {
    ...
    context_replacement: Mutex<Option<(usize, PendingOutputPoiContextRecord)>>,
}
...
impl RecordingCacheStore {
    fn new(db: Arc<DbStore>) -> Self {
        Self {
            db,
            state: Mutex::default(),
            context_replacement: Mutex::default(),
        }
    }

    fn replace_context_before_recovery_read(
        &self,
        call_index: usize,
        replacement: PendingOutputPoiContextRecord,
    ) {
        *self.context_replacement.lock().expect("replacement state") =
            Some((call_index, replacement));
    }
    ...
    fn fail_next_commit(&self) {
        self.state.lock().expect("cache state").fail_next_commit = true;
    }

    fn fail_pending_output_list_call(&self, call_index: usize) {
        self.state
            .lock()
            .expect("cache state")
            .fail_pending_output_list_call = Some(call_index);
    }

    fn fail_next_candidate_list(&self) {
        self.state
            .lock()
            .expect("cache state")
            .fail_next_candidate_list = true;
    }
...
impl WalletCacheStore for RecordingCacheStore {
    ...
    fn list_pending_output_poi_contexts(
        ...
        let mut state = self.state.lock().expect("cache state");
        state.pending_output_read_calls += 1;
        let call_index = state.pending_output_list_calls;
        state.pending_output_list_calls += 1;
        if state.fail_pending_output_list_call == Some(call_index) {
            state.fail_pending_output_list_call = None;
            return Err(WalletCacheError::Crypto);
        }
        drop(state);
    ...
    fn get_output_poi_recovery(
        ...
        let call_index = {
            let mut state = self.state.lock().expect("cache state");
            let call_index = state.output_recovery_read_calls;
            state.output_recovery_read_calls += 1;
            call_index
        };
        let replacement = {
            let mut replacement = self.context_replacement.lock().expect("replacement state");
            if replacement
                .as_ref()
                .is_some_and(|(replace_at, _)| *replace_at == call_index)
            {
                replacement.take().map(|(_, replacement)| replacement)
            } else {
                None
            }
        };
...
#[tokio::test]
async fn candidate_effect_commits_spent_snapshot_and_artifact_bound_atomically() {
    let root_dir = temp_db_root();
    let db = Arc::new(
    ...
    let wallet_id = test_cache_key("cache-key");
    let spent_source = source(0x51);
    let mut input = test_wallet_utxo(7);
    input.spent = Some(spent_source.clone());
    let snapshot = vec![input.clone()];
    let candidate = sender_candidate_for_spent_input(wallet_id.clone(), &input, spent_source);
    ...
    let full = persist_state
        .persist_progress_with_private_effects(
            &cache_store,
            &permit,
            WalletProgressPersist {
                cache_key: wallet_id.as_str(),
                snapshot: &snapshot,
                last_scanned: 10,
                checkpoint: WalletCheckpointMutation::Set {
                    last_scanned_block: 10,
                    last_scanned_block_hash: Some([0x10; 32]),
                },
                changed: false,
            },
            WalletProgressPrivateEffects {
                sender_transaction_candidate_updates: std::slice::from_ref(&candidate),
                ..WalletProgressPrivateEffects::default()
            },
        )
        .expect("persist candidate effects");

    assert!(full, "candidate effects require a full durable commit");
    assert_eq!(cache_store.state().store_calls, 1);
    let persisted = <DbStore as WalletCacheStore>::load_wallet_utxos(db.as_ref(), &wallet_id)
        .expect("load persisted snapshot");
    assert_eq!(persisted.len(), 1);
    assert_eq!(persisted[0].spent, input.spent);
    assert_eq!(
        <DbStore as WalletCacheStore>::get_wallet_meta(db.as_ref(), &wallet_id)
            .expect("load artifact_bound")
            .expect("artifact_bound present")
            .last_scanned_block,
        10
    );
    assert!(
    ...
    let mut replacement_source = candidate.source.clone();
    replacement_source.block_timestamp += 1;
    let mut replacement_input = input;
    replacement_input.spent = Some(replacement_source.clone());
    let replacement_snapshot = vec![replacement_input.clone()];
    let replacement =
        sender_candidate_for_spent_input(wallet_id.clone(), &replacement_input, replacement_source);
    for expected in [&replacement, &replacement] {
        persist_state
            .persist_progress_with_private_effects(
                &cache_store,
                &permit,
                ...
                WalletProgressPrivateEffects {
                    sender_transaction_candidate_updates: std::slice::from_ref(expected),
                    ..WalletProgressPrivateEffects::default()
                },
            )
            .expect("replace candidate evidence");
    }
    ...
    .expect("list replaced candidate");
    assert_eq!(candidates.len(), 1);
    assert_eq!(
        candidates[0].encode().expect("encode stored candidate"),
...
#[tokio::test]
async fn automatic_recovery_policy_uses_local_proofs_only_and_retains_candidate() {
    let root_dir = temp_db_root();
    let wallet_id = test_cache_key("candidate-maintenance-wallet");
    let spent_source = source(0x53);
    let mut input = test_wallet_utxo(9);
    input.spent = Some(spent_source.clone());
    let candidate = sender_candidate_for_spent_input(wallet_id.clone(), &input, spent_source);
    {
        ...
        .expect("persist candidate before restart");
    }
    ...
    cfg.cache_key = wallet_id.clone();
    let mut handle = test_wallet_handle(vec![input.clone()]);
    handle.cache_key = wallet_id.clone();
    let cancel = CancellationToken::new();
    let authority = WalletPrivateMutationAuthority::new(&handle, 0, &cancel);
    let rpc_url = Url::parse("http://127.0.0.1:1").expect("POI RPC URL");
    ...
    let resolver_forest = Arc::new(MerkleForest::new());
    let resolver_wallet_utxos = std::slice::from_ref(&input);
    // Owned-output and sender automatic recovery share this resolver; neither may use remote proofs.
    let proxy_resolver_request = OutputPoiRecoveryRequest {
        authority: &authority,
        db: db.as_ref(),
    ...
    assert!(matches!(
        proxy_resolver_request
            .resolve_proof_source(&active_list_keys)
            .await,
        OutputPoiProofSourceResolution::Unavailable
    ));
    let fallback_runtime = test_artifact_poi_runtime_with_fallback(rpc_url);
    let fallback_poi_client = fallback_runtime.public_client().clone();
    let fallback_resolver_request = OutputPoiRecoveryRequest {
        authority: &authority,
        db: db.as_ref(),
        cache_store: &cache_store,
        cfg: &cfg,
        ...
        indexed_artifact_source: None,
        forest: Arc::clone(&resolver_forest),
        poi_client: &fallback_poi_client,
        private_poi: &private_poi,
        poi_runtime: &fallback_runtime,
        active_list_keys: &active_list_keys,
        wallet_utxos: resolver_wallet_utxos,
        force_retry: false,
    };
    assert!(matches!(
        fallback_resolver_request
            .resolve_proof_source(&active_list_keys)
            .await,
        OutputPoiProofSourceResolution::Unavailable
    ));
    ...
    assert_eq!(recovered.recovered, 0, "the bounded fixture has no prover");
    assert!(recovered.error.is_none());
    assert_eq!(recovered.candidate_report.awaiting_poi_data, 1);
    assert_eq!(recovered.candidate_report.needs_attention, 0);
    assert_eq!(cache_store.state().candidate_list_calls, 1);
    assert!(
        cache_store
            .get_sender_transaction_candidate(1, &wallet_id, &candidate.semantic_id())
            .expect("reload candidate after maintenance")
            .is_some()
    );

    let fallback = (OutputPoiRecoveryRun {
        authority: &authority,
        db: db.as_ref(),
        cache_store: &cache_store,
        cfg: &cfg,
    ...
    .recover_missing()
    .await;
    assert_eq!(fallback.recovered, 0);
    assert_eq!(fallback.candidate_report.awaiting_poi_data, 1);
    assert_eq!(fallback.candidate_report.needs_attention, 0);
    assert!(
        cache_store
            .get_sender_transaction_candidate(1, &wallet_id, &candidate.semantic_id())
            .expect("reload fallback candidate")
            .is_some()
    );

    cache_store.fail_next_candidate_list();
    ...
    assert!(matches!(
        failed.error,
        Some(PoiMaintenanceError::SenderCandidateList(_))
    ));
    assert_eq!(cache_store.state().candidate_list_calls, 3);
...
fn recovery_input_tree_search_finds_root_beyond_former_bounded_window() {
    ...
    let Err(failure) = recovery_input_merkle_tree_for_root(
        &forest,
        first_input.utxo.tree,
        &first_input,
        256,
        ...
        panic!("unavailable historical root should fail");
    };
    assert_eq!(failure.status, OutputPoiRecoveryStatus::MissingMerkleProof);
    assert_eq!(failure.category, "input_merkle_proof_unavailable");
}
...
async fn output_poi_recovery_chunk_uses_public_row_and_sender_candidate_without_calldata() {
    ...
    .await
    .expect_err("missing public row must fail closed");
    assert_eq!(failure.status, OutputPoiRecoveryStatus::NotSelfOriginated);
    assert_eq!(failure.category, "public_row_not_unique_or_missing");
    ...
    assert_eq!(failure.category, "unshield_preimage_unavailable_or_invalid");
}
...
fn output_poi_recovery_reports_missing_private_output_indexes() {
    ...
    assert_eq!(failure.category, "missing_wallet_outputs");
...
async fn public_cache_txid_recovery_refreshes_stale_marker_for_known_target() {
    ...
    let recovery_graph_response = serde_json::json!({
        "data": {
            "transactions": [graph_response["data"]["transactions"][1].clone()]
        }
    });
    let seed_graph_response = seed_graph_response.to_string().into_bytes();
    let recovery_graph_response = recovery_graph_response.to_string().into_bytes();
    let (graph_endpoint, _graph_requests) = spawn_http_response(seed_graph_response).await;
    let (recovery_endpoint, recovery_requests) = spawn_http_response(recovery_graph_response).await;
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    assert!(matches!(
        direct_error,
        crate::txid_cache::TxidPublicCacheError::MissingTarget
    ...
    cfg.quick_sync_endpoint = Some(recovery_endpoint);
    ...
    let refresh_request = recovery_requests
        .recv_timeout(Duration::from_secs(2))
        .expect("recovery refresh request");
    assert!(refresh_request.contains("\"offset\":1"));
...
#[tokio::test]
async fn public_cache_txid_recovery_uses_artifact_bound_before_latest_refresh() {
    let bound_params_hash = U256::from(0x33_u64);
    let chunk = RecoveryChunk {
        chunk: railgun_wallet::tx::TransactionPlanChunk {
            tree_number: 0,
            ...
            public_inputs: PublicInputs {
                merkle_root: U256::ZERO,
                bound_params_hash,
                nullifiers: vec![U256::from(1_u64)],
                commitments_out: vec![U256::from(2_u64)],
            },
            private_inputs: PrivateInputs {
                token_address: U256::ZERO,
                random_in: Vec::new(),
                value_in: Vec::new(),
        ...
        output_start_global: 0,
        target_txid_index: Some(0),
    };
    let txid_transaction = IndexedRailgunTransaction {
        id: "artifact_bound".to_string(),
        block_number: U256::from(1_u64),
        block_timestamp: U256::from(1_700_000_001_u64),
        transaction_hash: FixedBytes::from([0x11; 32]),
        merkle_root: FixedBytes::from([0x22; 32]),
        nullifiers: vec![U256::from(1_u64)],
        commitments: vec![U256::from(2_u64)],
        bound_params_hash,
        has_unshield: false,
        unshield_token: None,
    ...
    let artifact_leaf =
        railgun_txid_leaf_hash_with_output_start(chunk.chunk.railgun_txid(), 0, U256::ZERO);
    let artifact_root = FixedBytes::from(
        DenseMerkleTree::from_ordered_leaves(vec![artifact_leaf], 1)
            .root()
            .to_be_bytes::<32>(),
    );
    let artifact_path = DenseMerkleTree::from_ordered_leaves(vec![artifact_leaf], 1)
        .prove(0)
        .path_elements
        .to_vec();
    let root_dir = temp_db_root();
    ...
    .await
    .expect("persist verified artifact artifact_bound");
    let public_data_plane =
        ChainPublicDataPlane::new(Arc::clone(&store), Arc::new(AtomicU64::new(0)));
    let mut cfg = wallet_config(U256::ZERO);
...
async fn external_local_valid_context_completes_without_submission() {
    ...
    let pending =
        external_pending_output_record(&cfg, 0x42, list_key, PendingOutputPoiRole::Recipient);
    let identity = pending_output_poi_submit_identity(
        &pending,
        pending
            .observation
            .as_ref()
            .expect("observed external context"),
    )
    .expect("external submit identity");
    store
        .put_pending_output_poi_context(&pending)
        .expect("store external context");
    ...
    assert!(
        store
            ...
            .expect("load completed external context")
            .is_none()
    );
    assert!(
        store
...
async fn partial_local_validity_submits_only_unresolved_list() {
    ...
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-partial-local-valid");
    let wallet_utxo = test_wallet_utxo(43);
    let mut pending = matching_pending_output_record(&cfg, &wallet_utxo, list_a);
    pending.required_poi_list_keys.push(list_b);
    pending.pre_transaction_pois_per_txid_leaf_per_list.insert(
        list_b,
        BTreeMap::from([(FixedBytes::from([0x77; 32]), sample_pre_tx_poi(0x77))]),
    );
    store
        .put_pending_output_poi_context(&pending)
        .expect("store partial context");
    let mut handle = test_wallet_handle(vec![wallet_utxo.clone()]);
    handle.cache_key = cfg.cache_key.clone();
    let cancel = CancellationToken::new();
    let authority = WalletPrivateMutationAuthority::new(&handle, 0, &cancel);
    ...
    assert_eq!(
        handle.utxos.read().await[0].utxo.poi.statuses.get(&list_a),
        Some(&PoiStatus::Valid)
    );
...
async fn local_missing_status_keeps_canonical_submission_eligible() {
    ...
    let wallet_utxo = test_wallet_utxo(45);
    let pending = matching_pending_output_record(&cfg, &wallet_utxo, list_key);
    store
        .put_pending_output_poi_context(&pending)
        .expect("store missing context");
    let mut handle = test_wallet_handle(vec![wallet_utxo.clone()]);
    handle.cache_key = cfg.cache_key.clone();
    let cancel = CancellationToken::new();
    let authority = WalletPrivateMutationAuthority::new(&handle, 0, &cancel);
    let reader =
        local_status_reader_for_lists(wallet_utxo.utxo.poi.blinded_commitment, &[list_key], &[]);
    let outcome = super::pending_output_poi::reconcile_pending_output_poi_local_statuses(
        &authority,
        &reader,
        &store,
...
#[tokio::test]
async fn sender_candidate_batch_refreshes_validated_txid_coverage_once() {
    let list_key = FixedBytes::from([0x31; 32]);
    let mut input = test_wallet_utxo(0x32);
    let candidate = sender_candidate_for_spent_input(
        WalletCacheKey::from_opaque_bytes(b"batch-refresh-candidate").expect("cache key"),
        &input,
        source(0x33),
    );
    input.spent = Some(candidate.source.clone());
    let second_candidate =
        sender_candidate_for_spent_input(candidate.wallet_id.clone(), &input, source(0x34));
    let mut cfg = wallet_config(input.utxo.nullifier(U256::ZERO));
    cfg.cache_key = candidate.wallet_id.clone();
    cfg.scan_keys.nullifying_key = U256::ZERO;
    let root_dir = temp_db_root();
    let db = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    for persisted in [&candidate, &second_candidate] {
        db.put_opaque_wallet_private_row(
            &persisted.namespace(),
            WalletPrivateRecordKind::SenderTransactionCandidate,
            &OpaqueWalletPrivateRow {
                row_id: persisted.row_identity(),
                payload: persisted.encode().expect("encode candidate"),
            },
        )
        .expect("persist candidate");
    }
    cfg.spending_public_key = Some([U256::ZERO; 2]);
    ...
    poi_cache
        ...
        .expect("apply unrelated output event");
    poi_cache.accept_current_roots();
    let public_data_plane = test_public_data_plane_with_poi_service(&db);
    seed_data_plane_poi_cache(&public_data_plane, cfg.chain.chain_id, list_key, poi_cache).await;
    let foreign_graph_response = serde_json::json!({
        "data": {
            "transactions": [{
                "id": "0x00",
                "blockNumber": "1",
    ...
    .to_string()
    .into_bytes();
    let (graph_endpoint, _graph_requests) = spawn_http_response(foreign_graph_response).await;
    public_data_plane
        .sync_txid_public_cache(PublicTxidSyncRequest {
    ...
    let report = materialize_sender_transaction_candidates(SenderCandidateRecoveryRequest {
        ...
        candidates: vec![candidate, second_candidate],
    })
    .await;

    assert_eq!(
        report.awaiting_public_txid_data, 2,
...
#[tokio::test]
async fn locally_valid_sender_candidate_retires_without_public_or_remote_effects() {
    let root_dir = temp_db_root();
    let db = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let mut cfg = wallet_config(U256::from(17));
    cfg.cache_key = test_cache_key("local-retirement");
    let active_list_keys = vec![FixedBytes::from([0x41; 32]), FixedBytes::from([0x42; 32])];
    let mut input = test_wallet_utxo(1);
    let source = source(0x71);
    input.spent = Some(source.clone());
    let external_note = Note {
        token_hash: U256::from(7),
        value: U256::from(9),
        random: [0x5a; 16],
        npk: U256::from(11),
    ...
    let external_output = SenderTransactionCandidateOutput {
        tree: 3,
        position: 4,
        commitment: FixedBytes::from(external_note.commitment().to_be_bytes::<32>()),
        note: Some(external_note),
    };
    ...
    let candidate = SenderTransactionCandidate::new(
        ...
        vec![external_output.clone(), owned_output.clone()],
    )
    .expect("local-only candidate");
    let owned_change = WalletUtxo {
        utxo: Utxo::new(
            owned_note,
            owned_output.tree,
    ...
    let wallet_utxos = vec![input.clone(), owned_change];
    let expected_wallet_utxos = wallet_utxos.clone();
    db.put_opaque_wallet_private_row(
        &candidate.namespace(),
        WalletPrivateRecordKind::SenderTransactionCandidate,
        &OpaqueWalletPrivateRow {
    ...
    .expect("persist candidate");
    let mut pending_owned_context = pending_output_record(
        cfg.chain.chain_id,
        owned_output.commitment,
        active_list_keys[0],
    );
    pending_owned_context.wallet_id = cfg.cache_key.to_string();
    pending_owned_context.output_npk = FixedBytes::from(
        owned_output
            .note
            .as_ref()
            .expect("owned output note")
            .npk
            .to_be_bytes::<32>(),
    );
    pending_owned_context.observation = Some(local_db::PendingOutputPoiObservation {
        output_tree: u64::from(owned_output.tree),
        output_position: owned_output.position,
    ...
    let second_list_key = active_list_keys[1];
    pending_owned_context
        .required_poi_list_keys
        .push(second_list_key);
    pending_owned_context
        .pre_transaction_pois_per_txid_leaf_per_list
        .insert(
            second_list_key,
            BTreeMap::from([(FixedBytes::from([0x56; 32]), sample_pre_tx_poi(0x11))]),
        );
    db.put_pending_output_poi_context(&pending_owned_context)
        .expect("persist pending owned context");
    let external_blinded_commitment = Utxo::new(
        external_output.note.clone().expect("external note"),
        external_output.tree,
        external_output.position,
        candidate.source.clone(),
        UtxoCommitmentKind::Transact,
    )
    .poi
    ...
    for list_key in active_list_keys.iter().copied() {
        ...
        cache
            .apply_verified_artifact_events(&[poi::artifacts::SnapshotEvent {
                event_index: 0,
                blinded_commitment: external_blinded_commitment.0,
                signature: [0_u8; 64],
                event_type: PoiEventType::Transact,
            }])
            .expect("seed valid local status");
        cache.accept_current_roots();
        seed_data_plane_poi_cache(&public_data_plane, cfg.chain.chain_id, list_key, cache).await;
    }
    ...
    handle.cache_key = cfg.cache_key.clone();
    let cancel = CancellationToken::new();
    let remote_status = Arc::new(RecordingPoiStatusClient::default());
    let remote_submit = Arc::new(RecordingPendingOutputPoiSubmitter::default());
    let private_poi = WalletPrivatePoiClients::for_test(
        WalletPrivateMutationAuthority::new(&handle, 0, &cancel).remote_authority(),
        remote_status.clone(),
        remote_submit.clone(),
    );
    let poi_runtime = test_artifact_poi_runtime();
    ...
    let mixed_report = materialize_sender_transaction_candidates(SenderCandidateRecoveryRequest {
        output_recovery: OutputPoiRecoveryRequest {
            authority: &authority,
            db: db.as_ref(),
            cache_store: db.as_ref(),
    ...
    assert_eq!(mixed_report.covered_by_pending_contexts, 1);
    assert_eq!(mixed_report.materialized, 0);
    assert_eq!(mixed_report.retired_locally_valid, 0);
    ...
    assert!(
        ...
        .expect("load covered candidate")
        .is_some()
    );
    assert!(remote_status.calls().is_empty());
    assert!(remote_submit.calls().is_empty());
    db.delete_pending_output_poi_context(
        cfg.chain.chain_id,
        cfg.cache_key.as_str(),
        &owned_output.commitment,
    )
    .expect("remove pending owned context");

    let actor_handle = handle.clone();
    let actor_db = Arc::clone(&db);
    let actor_cancel = cancel.clone();
    let actor_cfg = cfg.clone();
    let actor_active_list_keys = active_list_keys.clone();
    let actor = async move {
        let request = apply_rx
            .recv()
            .await
            .expect("local retirement actor request");
        let result = apply_owned_poi_private_delta_on_actor_with_active_lists(
            ...
            actor_db.as_ref(),
            &actor_cfg,
            &actor_active_list_keys,
            request.delta,
        )
        .await;
        request.reply.send(result).expect("send actor result");
    };
    let recovery = materialize_sender_transaction_candidates(SenderCandidateRecoveryRequest {
        output_recovery: OutputPoiRecoveryRequest {
            authority: &authority,
            db: db.as_ref(),
            cache_store: db.as_ref(),
    ...
    let (report, ()) = tokio::join!(recovery, actor);

    assert_eq!(report.retired_locally_valid, 1);
    ...
    assert!(
        ...
        .expect("load retired candidate")
        .is_none()
    );
    let current_wallet_utxos = handle.utxos.read().await;
    assert_eq!(current_wallet_utxos.len(), expected_wallet_utxos.len());
    for (current, expected) in current_wallet_utxos.iter().zip(&expected_wallet_utxos) {
        assert_eq!(
            wallet_utxo_stable_identity(current),
            wallet_utxo_stable_identity(expected)
        );
        assert_eq!(current.spent, expected.spent);
    }
    drop(current_wallet_utxos);
    assert!(
    ...
    assert!(
        db.list_output_poi_recoveries(cfg.chain.chain_id, &cfg.cache_key)
            .expect("list recovery rows")
            .is_empty()
    );
    let observation = handle.observation();
    let workflow_status = observation.ppoi_workflow_status();
    assert_eq!(workflow_status.awaiting_recovery, 0);
    assert_eq!(workflow_status.outstanding_count(), 0);
    assert!(remote_status.calls().is_empty());
    assert!(remote_submit.calls().is_empty());
    ...
    db.put_opaque_wallet_private_row(
        ...
        &OpaqueWalletPrivateRow {
            row_id: candidate.row_identity(),
            payload: candidate.encode().expect("re-encode candidate"),
        },
    )
    ...
    let corpus = public_data_plane
        ...
        .await
        .expect("reload local corpus");
    let mut missing_cache = PoiCache::new(PoiCacheIdentity::new(
        EVM_CHAIN_TYPE,
        cfg.chain.chain_id,
        DEFAULT_TXID_VERSION,
        active_list_keys[1],
    ...
    missing_cache
        ...
        .expect("seed missing local status");
    missing_cache.accept_current_roots();
    corpus
        .local_caches()
        .write()
        .await
        .insert(active_list_keys[1], missing_cache);
    ...
    .await;
    assert_eq!(report.retired_locally_valid, 0);
    assert_eq!(report.materialized, 0);
...
#[tokio::test]
async fn partial_outer_txid_coverage_refreshes_before_candidate_attention() {
    let list_key = FixedBytes::from([0x41; 32]);
    let mut first_input = test_wallet_utxo(1);
    let mut second_input = test_wallet_utxo(2);
    let candidate_source = source(0x42);
    first_input.spent = Some(candidate_source.clone());
    second_input.spent = Some(candidate_source.clone());
    let first_note = Note {
        token_hash: U256::from(7),
        value: U256::from(9),
        random: [0x43; 16],
    ...
    let candidate = SenderTransactionCandidate::new(
        1,
        WalletCacheKey::from_opaque_bytes(b"partial-outer-txid-candidate").expect("cache key"),
        candidate_source.clone(),
        vec![
        ...
        vec![
            SenderTransactionCandidateOutput {
                tree: 3,
                position: 4,
                commitment: FixedBytes::from(first_note.commitment().to_be_bytes::<32>()),
                note: Some(first_note),
            },
            SenderTransactionCandidateOutput {
                tree: 3,
                position: 5,
    ...
    .expect("partial-coverage candidate");
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = candidate.wallet_id.clone();
    cfg.spending_public_key = Some([U256::ZERO; 2]);
    let root_dir = temp_db_root();
    ...
    let public_data_plane = test_public_data_plane_with_poi_service(&db);
    seed_data_plane_poi_cache(&public_data_plane, cfg.chain.chain_id, list_key, poi_cache).await;
    let txid_row = |id: &str, input: &WalletUtxo, output: &SenderTransactionCandidateOutput| {
        serde_json::json!({
            "id": id,
            "blockNumber": candidate.source.block_number.to_string(),
            ...
            "commitments": [hex::encode_prefixed(output.commitment)],
            "boundParamsHash": "0x02",
            "hasUnshield": false,
            "utxoTreeIn": input.utxo.tree.to_string(),
    ...
    let stale_graph_response = serde_json::json!({
        "data": { "transactions": [txid_row("0x00", &first_input, &candidate.outputs[0])] }
    })
    .to_string()
    .into_bytes();
    let refresh_graph_response = serde_json::json!({
        "data": { "transactions": [txid_row("0x01", &second_input, &candidate.outputs[1])] }
    })
    .to_string()
    .into_bytes();
    let (stale_graph_endpoint, _stale_requests) = spawn_http_response(stale_graph_response).await;
    let cache_key = DataPlanePublicTxidCacheKey::new(
        ChainScope {
            chain_type: ChainType::Evm,
            chain_id: cfg.chain.chain_id,
    ...
    public_data_plane
        ...
        .await
        .expect("seed partial validated TXID coverage");
    let wallet_utxos = vec![first_input, second_input];
    let mut handle = test_wallet_handle(wallet_utxos.clone());
    handle.cache_key = cfg.cache_key.clone();
    let cancel = CancellationToken::new();
    let authority = WalletPrivateMutationAuthority::new(&handle, 0, &cancel);
    ...
    .await;
    assert_eq!(
        still_partial.awaiting_public_txid_data, 2,
    ...
    assert_eq!(report.needs_attention, 0, "unexpected report: {report:?}");
    assert_eq!(report.retrying, 2, "unexpected report: {report:?}");
    assert!(
        refresh_requests
            .recv_timeout(Duration::from_secs(2))
            .is_ok()
...
async fn assert_stale_output_recovery_source_stops_before_transport(cached_input: bool) {
    ...
    assert_eq!(failure.category, "tx_fetch_failed");
    assert!(
        failure
            .message
...
async fn live_poi_tail_applies_public_leaves_and_validates_root() {
    ...
    expected_cache
        ...
        .expect("apply artifact artifact_bound event");
    expected_cache
        .apply_verified_artifact_events(&[poi::artifacts::SnapshotEvent {
            event_index: 1,
            blinded_commitment: *blinded_commitment,
...
#[tokio::test]
async fn pending_tip_tentative_submission_is_non_durable_and_safe_head_remains_eligible() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let list_key = FixedBytes::from([0x45; 32]);
    let observed_source = source(8);
    let mut record = pending_output_record(cfg.chain.chain_id, output_commitment, list_key);
    record.wallet_id = cfg.cache_key.to_string();
    ...
    let candidates = prepare_pending_output_poi_tentative_candidates(
        store.as_ref(),
        &cfg,
        &[list_key],
        std::slice::from_ref(&observation),
    )
    .expect("prepare tentative candidate");
    assert_eq!(
        candidates.len(),
        1,
        "matching observation must be a candidate"
    );
    let candidate_key = candidates[0].attempt_key();

    let nonmatching = CommitmentObservation {
        commitment: U256::from_be_bytes([0x79; 32]),
        ..observation.clone()
    };
    assert!(
        prepare_pending_output_poi_tentative_candidates(
        ...
        .expect("prepare nonmatching candidate")
        .is_empty()
    );
    ...
    let private_poi =
        WalletPrivatePoiClients::for_submit(authority.remote_authority(), submitter.clone());
    submitter.fail_next_missing_railgun_txid();
    let failed = submit_pending_output_poi_tentative_candidates(
        &authority,
        store.as_ref(),
        &cfg,
        &[list_key],
    ...
    .await;
    assert!(failed.succeeded.is_empty());
    assert_eq!(failed.retryable, vec![candidate_key.clone()]);
    assert_eq!(failed.accepted, 0);
    assert_eq!(failed.missing_txid, 1);
    assert_eq!(failed.remote_failure, 0);
    assert_eq!(failed.context_stale, 0);
    assert!(!failed.authority_stale);
    let after_tentative = store
        .get_pending_output_poi_context(cfg.chain.chain_id, &cfg.cache_key, &output_commitment)
        .expect("load context after tentative submission")
        .expect("context after tentative submission");
    assert!(after_tentative.observation.is_none());
    assert!(after_tentative.submitted_poi_list_keys.is_empty());
    assert!(after_tentative.terminal_error.is_none());
    assert!(
        store
            .get_output_poi_recovery(cfg.chain.chain_id, &cfg.cache_key, &output_commitment)
            .expect("load recovery after tentative submission")
            .is_none()
    );

    submitter.fail_next_other_remote_error();
    let remote_failed = submit_pending_output_poi_tentative_candidates(
        &authority,
        store.as_ref(),
        &cfg,
        &[list_key],
    ...
    .await;
    assert!(remote_failed.succeeded.is_empty());
    assert_eq!(remote_failed.retryable, vec![candidate_key.clone()]);
    assert_eq!(remote_failed.accepted, 0);
    assert_eq!(remote_failed.missing_txid, 0);
    assert_eq!(remote_failed.remote_failure, 1);
    assert_eq!(remote_failed.context_stale, 0);
    assert!(!remote_failed.authority_stale);

    let succeeded = submit_pending_output_poi_tentative_candidates(
        &authority,
        store.as_ref(),
        &cfg,
        &[list_key],
    ...
    .await;
    assert_eq!(succeeded.succeeded, vec![candidate_key.clone()]);
    assert!(succeeded.retryable.is_empty());
    assert_eq!(succeeded.accepted, 1);
    assert_eq!(succeeded.missing_txid, 0);
    assert_eq!(succeeded.remote_failure, 0);
    assert_eq!(succeeded.context_stale, 0);
    assert!(!succeeded.authority_stale);
    let after_success = store
        .get_pending_output_poi_context(cfg.chain.chain_id, &cfg.cache_key, &output_commitment)
        .expect("load context after tentative success")
        .expect("context after tentative success");
    assert!(after_success.observation.is_none());
    assert!(after_success.submitted_poi_list_keys.is_empty());
    assert!(after_success.terminal_error.is_none());
    assert!(
        store
            .get_output_poi_recovery(cfg.chain.chain_id, &cfg.cache_key, &output_commitment)
            .expect("load recovery after tentative failure")
            .is_none()
    );

    let changed_observation = CommitmentObservation {
        position: 35,
        ..observation.clone()
    };
    let changed_candidates = prepare_pending_output_poi_tentative_candidates(
        store.as_ref(),
        &cfg,
        &[list_key],
    ...
    assert_eq!(changed_candidates.len(), 1);
    assert_ne!(candidate_key, changed_candidates[0].attempt_key());

    let updates = pending_output_poi_observation_state_updates(
        store.as_ref(),
        cfg.chain.chain_id,
        &cfg.cache_key,
        std::slice::from_ref(&observation),
    )
    .expect("prepare safe-head observation");
    for output_commitment in updates.recovery_deletes {
        store
            .delete_output_poi_recovery(cfg.chain.chain_id, &cfg.cache_key, &output_commitment)
            .expect("delete safe-head recovery");
    }
    for record in updates.context_updates {
        store
            .put_pending_output_poi_context(&record)
            .expect("persist safe-head observation");
    }
    ...
    .await
    .expect("safe-head submission");
    assert_eq!(submitted, 1, "safe-head path must remain eligible");
    assert_eq!(
        store
            .get_pending_output_poi_context(cfg.chain.chain_id, &cfg.cache_key, &output_commitment)
            .expect("load safe-head context")
            .expect("safe-head context")
...
#[tokio::test]
async fn proxy_refresh_reconciles_owned_valid_context_after_submission_persistence_failure() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    handle
        .observation
        .upgrade()
        .expect("observation publisher")
        .publish_ppoi_workflow_status(WalletPpoiWorkflowStatus {
    ...
    assert!(
        store
            ...
            .expect("load pending context")
            .expect("pending context preserved")
            .submitted_poi_list_keys
            .is_empty()
    );
    assert!(
    ...
    assert_eq!(
        handle.observation().ppoi_workflow_status(),
        &WalletPpoiWorkflowStatus {
            awaiting_submission: 1,
            ..WalletPpoiWorkflowStatus::default()
        }
...
#[tokio::test]
async fn verified_valid_rejects_mismatched_recovery_source() {
    let root_dir = temp_db_root();
    let store = DbStore::open(DbConfig {
        root_dir: root_dir.clone(),
    })
    ...
    let recovery = store
        ...
        .expect("load recovery")
        .expect("recovery present");
    assert_eq!(recovery.source_tx_hash, stale_recovery.source_tx_hash);
    assert_eq!(recovery.status, stale_recovery.status);
    assert_eq!(recovery.tx_input, stale_recovery.tx_input);
    assert_eq!(recovery.attempt_count, stale_recovery.attempt_count);
    assert_eq!(
        handle.utxos.read().await[0]
            .utxo
            .poi
...
async fn recovered_outgoing_siblings_submit_once_and_preserve_ordinary_behavior() {
    ...
    let group = recovered_outgoing_pending_group(&cfg, list_key);
    seed_recovered_outgoing_pending_group(&store, &cfg, &group);
    let ordinary =
        external_pending_output_record(&cfg, 0xd5, list_key, PendingOutputPoiRole::Recipient);
    store
        .put_pending_output_poi_context(&ordinary)
        .expect("store ordinary context");
    let mut handle = test_wallet_handle(Vec::new());
    handle.cache_key = cfg.cache_key.clone();
    let cancel = CancellationToken::new();
    let authority = WalletPrivateMutationAuthority::new(&handle, 0, &cancel);
    ...
    assert_eq!(
        submitter.calls().len(),
        2,
        "one grouped and one ordinary RPC"
    );
    for record in group.iter().chain(std::iter::once(&ordinary)) {
        let current = store
            .get_pending_output_poi_context(
                cfg.chain.chain_id,
                &cfg.cache_key,
            ...
            .expect("context remains pending validation");
        assert_eq!(current.submitted_poi_list_keys, vec![list_key]);
        assert_eq!(
            store
                .get_output_poi_recovery(
                    cfg.chain.chain_id,
    ...
    for case in ["valid", "missing", "spent", "source", "status", "stale"] {
        ...
        store
            .delete_pending_output_poi_context(
                cfg.chain.chain_id,
                cfg.cache_key.as_str(),
                &group[1].output_commitment,
            )
            .expect("retire owned pending context");
        store
            ...
            .expect("retire owned recovery");

        let mut owned = owned_substitute_for_recovered_context(&group[1], list_key);
        match case {
            "missing" => owned.utxo.poi.blinded_commitment = FixedBytes::from([0xfe; 32]),
            "spent" => owned.spent = Some(source(0xfe)),
            "source" => owned.utxo.source.block_number += 1,
            "status" => {
                owned.utxo.poi.statuses.insert(list_key, PoiStatus::Missing);
            }
            "valid" | "stale" => {}
            _ => unreachable!(),
        }
        let mut handle = test_wallet_handle(vec![owned]);
        handle.cache_key = cfg.cache_key.clone();
        let cancel = CancellationToken::new();
        let authority = WalletPrivateMutationAuthority::new(&handle, 0, &cancel);

        let calls = if case == "stale" {
            let submitter = Arc::new(InvalidateOwnedSubstituteAfterSubmitter {
            ...
            submitter.calls.load(Ordering::Acquire)
        } else {
            let submitter = Arc::new(RecordingPendingOutputPoiSubmitter::default());
            let private_poi = WalletPrivatePoiClients::for_submit(
                authority.remote_authority(),
            ...
            .await;
            assert_eq!(submitted, usize::from(case == "valid"), "case={case}");
            u64::try_from(submitter.calls().len()).expect("bounded calls")
        };

        assert_eq!(
            calls,
            u64::from(matches!(case, "valid" | "stale")),
            "case={case}"
        );
        let external = store
            .get_pending_output_poi_context(
                cfg.chain.chain_id,
                &cfg.cache_key,
                &group[0].output_commitment,
            )
            .expect("load external context")
            .expect("external context retained");
        assert_eq!(
            external.submitted_poi_list_keys,
            if case == "valid" {
                vec![list_key]
            } else {
                Vec::new()
        ...
        assert!(
            store
                ...
                .expect("load retired owned context")
                .is_none(),
            "case={case}"
        );
        assert!(
            store
                ...
                .expect("load retired owned recovery")
                .is_none(),
            "case={case}"
        );
...
async fn recovered_outgoing_group_fails_closed_before_rpc() {
    for case in ["missing", "mismatched", "stale-predecessor"] {
        ...
        let mut group = recovered_outgoing_pending_group(&cfg, list_key);
        if case == "mismatched" {
            group[1].railgun_txid = U256::from(8);
        }
        let seeded = if case == "missing" {
            &group[..1]
        } else {
            group.as_slice()
        };
        seed_recovered_outgoing_pending_group(&store, &cfg, seeded);
        if case == "stale-predecessor" {
            let mut recovery = store
                ...
                .expect("load recovery")
                .expect("recovery present");
            recovery.source_tx_hash = FixedBytes::from([0xff; 32]);
            store
                .put_output_poi_recovery(&recovery)
                .expect("replace recovery predecessor");
        }
        let mut handle = test_wallet_handle(Vec::new());
        handle.cache_key = cfg.cache_key.clone();
        let cancel = CancellationToken::new();
        ...
        assert!(submitter.calls().is_empty(), "case={case}");
...
#[tokio::test]
async fn recovered_outgoing_group_final_apply_rechecks_every_predecessor() {
    let root_dir = temp_db_root();
    let store = DbStore::open(DbConfig {
        root_dir: root_dir.clone(),
    })
    .expect("open db");
    let list_key = FixedBytes::from([0xda; 32]);
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-recovered-outgoing-final-fence");
    let group = recovered_outgoing_pending_group(&cfg, list_key);
    seed_recovered_outgoing_pending_group(&store, &cfg, &group);
    let siblings = group
        .iter()
        .map(|context| {
            let observation = context.observation.as_ref().expect("group observation");
            let identity = pending_output_poi_submit_identity(context, observation)
                .expect("group submit identity");
            let recovery = store
                .get_output_poi_recovery(
                    cfg.chain.chain_id,
                ...
                .expect("load recovery")
                .expect("recovery present");
            RecoveredOutgoingSubmissionSibling {
                subject: PendingOutputPoiSubject::External {
                    output_commitment: context.output_commitment,
        ...
        .collect::<Vec<_>>();
    let mut replacement = group[1].clone();
    replacement.source_operation_id = Some("replacement-after-submit".to_string());
    store
        .put_pending_output_poi_context(&replacement)
        .expect("replace one grouped predecessor");
    let mut handle = test_wallet_handle(Vec::new());
    handle.cache_key = cfg.cache_key.clone();
    let cancel = CancellationToken::new();
    ...
    .await
    .expect("reject stale grouped final apply");
    ...
    for context in &group {
        let current = store
            ...
            .expect("load unchanged group context")
            .expect("group context present");
        ...
        assert_eq!(
            store
                ...
                .expect("load unchanged group recovery")
                .expect("group recovery present")
                .status,
            OutputPoiRecoveryStatus::Recoverable
        );
    }
...
#[tokio::test]
async fn recovered_outgoing_group_read_error_prevents_rpc() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let cache_store = RecordingCacheStore::new(Arc::clone(&store));
    let list_key = FixedBytes::from([0xd9; 32]);
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-recovered-outgoing-read-error");
    let group = recovered_outgoing_pending_group(&cfg, list_key);
    seed_recovered_outgoing_pending_group(store.as_ref(), &cfg, &group);
    cache_store.fail_pending_output_list_call(1);
    let mut handle = test_wallet_handle(Vec::new());
    handle.cache_key = cfg.cache_key.clone();
    let cancel = CancellationToken::new();
    let authority = WalletPrivateMutationAuthority::new(&handle, 0, &cancel);
...
#[tokio::test]
async fn recovered_outgoing_group_persistence_failure_has_no_partial_apply() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let cache_store = RecordingCacheStore::new(Arc::clone(&store));
    let list_key = FixedBytes::from([0xd7; 32]);
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-recovered-outgoing-persist-failure");
    let group = recovered_outgoing_pending_group(&cfg, list_key);
    seed_recovered_outgoing_pending_group(store.as_ref(), &cfg, &group);
    cache_store.fail_next_commit();
    let mut handle = test_wallet_handle(Vec::new());
...
async fn recovered_outgoing_ambiguous_remote_failure_stays_coherently_retryable() {
    ...
    for record in &group {
        let context = store
            ...
            .expect("load retryable context")
            .expect("context present");
        assert!(context.submitted_poi_list_keys.is_empty());
        let recovery = store
            .get_output_poi_recovery(
                cfg.chain.chain_id,
                &cfg.cache_key,
            ...
            .expect("load retryable recovery")
            .expect("recovery present");
...
#[tokio::test]
async fn authorized_external_pending_outputs_submit_without_sender_utxos() {
    for (byte, role) in [
        (0xa4_u8, PendingOutputPoiRole::Recipient),
        ...
        let pending = external_pending_output_record(&cfg, byte, list_key, role);
        store
            .put_pending_output_poi_context(&pending)
        ...
        let persisted = store
            ...
            .expect("load external pending context")
            .expect("external context remains pending verification");
        assert_eq!(persisted.submitted_poi_list_keys, vec![list_key]);
        let recovery = store
            .get_output_poi_recovery(
                cfg.chain.chain_id,
                &cfg.cache_key,
            ...
            .expect("load external recovery")
            .expect("external recovery present");
...
#[tokio::test]
async fn external_submission_rejects_scan_replacement_before_remote_transport() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let cache_store = RecordingCacheStore::new(Arc::clone(&store));
    let list_key = FixedBytes::from([0xb1; 32]);
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-external-submit-scan-replacement");
    let pending =
        external_pending_output_record(&cfg, 0xb0, list_key, PendingOutputPoiRole::Recipient);
    store
        .put_pending_output_poi_context(&pending)
        .expect("store external pending context");
    let mut replacement = pending.clone();
    let replacement_observation = replacement.observation.as_mut().expect("observation");
    replacement_observation.tx_hash = FixedBytes::from([0xb2; 32]);
    replacement_observation.block_number += 1;
    cache_store.replace_context_before_recovery_read(3, replacement.clone());
    ...
    let identity = pending_output_poi_submit_identity(&pending, &observation).expect("identity");
    let subject = PendingOutputPoiSubject::External {
    ...
    assert_eq!(
        ...
        pending_output_poi_context_fingerprint(&replacement),
    );
...
async fn authorized_external_pending_output_force_retry_resubmits_without_sender_utxo() {
    ...
    let mut pending =
        external_pending_output_record(&cfg, 0xa6, list_key, PendingOutputPoiRole::Recipient);
    pending.submitted_poi_list_keys = vec![list_key];
    store
        .put_pending_output_poi_context(&pending)
        .expect("store external pending context");
    ...
    recovery.source_tx_hash = pending.observation.as_ref().expect("observation").tx_hash;
    store
        .put_output_poi_recovery(&recovery)
        .expect("store external recovery");
    let mut handle = test_wallet_handle(Vec::new());
    handle.cache_key = cfg.cache_key.clone();
    let cancel = CancellationToken::new();
    let authority = WalletPrivateMutationAuthority::new(&handle, 0, &cancel);
    ...
    assert_eq!(submitter.calls().len(), 1);
    let empty_utxos = Arc::new(RwLock::new(Vec::new()));
    assert_eq!(
...
#[tokio::test]
async fn authorized_external_pending_output_verification_retires_private_state() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let list_key = FixedBytes::from([0xa9; 32]);
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-external-verification");
    let mut pending =
...
#[tokio::test]
async fn external_proxy_verification_rejects_scan_replacement_before_remote_transport() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-external-verify-scan-replacement");
    let mut pending =
        external_pending_output_record(&cfg, 0xb3, list_key, PendingOutputPoiRole::BroadcasterFee);
    pending.submitted_poi_list_keys = vec![list_key];
    store
        .put_pending_output_poi_context(&pending)
        .expect("store external pending context");
    let mut replacement = pending.clone();
    let replacement_observation = replacement.observation.as_mut().expect("observation");
    replacement_observation.tx_hash = FixedBytes::from([0xb5; 32]);
    replacement_observation.block_number += 1;
    replacement.submitted_poi_list_keys.clear();
    cache_store.replace_context_before_recovery_read(2, replacement.clone());
...
#[tokio::test]
async fn submitted_external_proxy_verification_rejects_mismatched_recovery_before_remote() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let list_key = FixedBytes::from([0xab; 32]);
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-stale-external-proxy-verification");
    let mut pending =
        external_pending_output_record(&cfg, 0xaa, list_key, PendingOutputPoiRole::Recipient);
    pending.submitted_poi_list_keys = vec![list_key];
    store
        .put_pending_output_poi_context(&pending)
        .expect("store external pending context");
    let stale_recovery = output_poi_recovery_record(
        cfg.chain.chain_id,
        &cfg.cache_key,
        pending.output_commitment,
        OutputPoiRecoveryStatus::Submitted,
...
#[tokio::test]
async fn unsubmitted_external_pending_output_cannot_use_owned_valid_reconciliation() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-unsubmitted-external-verification");
    let pending =
        external_pending_output_record(&cfg, 0xa9, list_key, PendingOutputPoiRole::Recipient);
    store
        .put_pending_output_poi_context(&pending)
        .expect("store external pending context");
    let mut handle = test_wallet_handle(Vec::new());
...
#[tokio::test]
async fn authorized_external_pending_output_persistence_failure_does_not_commit() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let list_key = FixedBytes::from([0xab; 32]);
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-external-persist-failure");
    let pending =
        external_pending_output_record(&cfg, 0xaa, list_key, PendingOutputPoiRole::Recipient);
    store
        .put_pending_output_poi_context(&pending)
    ...
    let mut handle = test_wallet_handle(Vec::new());
    handle.cache_key = cfg.cache_key.clone();
    let workflow_before = WalletPpoiWorkflowStatus {
        awaiting_validation: 1,
    ...
    handle
        ...
        .upgrade()
        .expect("observation publisher")
        .publish_ppoi_workflow_status(workflow_before);
    let cancel = CancellationToken::new();
    let authority = WalletPrivateMutationAuthority::new(&handle, 0, &cancel);
    let submitter = Arc::new(RecordingPendingOutputPoiSubmitter::default());
    let private_poi =
...
#[tokio::test]
async fn authorized_external_verification_persistence_failure_does_not_retire_context() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let cache_store = RecordingCacheStore::new(Arc::clone(&store));
    let list_key = FixedBytes::from([0xad; 32]);
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-external-verify-persist-failure");
    let mut pending =
        external_pending_output_record(&cfg, 0xac, list_key, PendingOutputPoiRole::BroadcasterFee);
    pending.submitted_poi_list_keys = vec![list_key];
    store
        .put_pending_output_poi_context(&pending)
        .expect("store external pending context");
    ...
    let outcome = verify_submitted_pending_output_pois_authorized_with_projection(
        &authority,
        &RecordingPoiStatusClient::default(),
        store.as_ref(),
        &cfg,
        &cache_store,
        &[list_key],
    ...
    let persisted_recovery = store
        ...
        .expect("load recovery")
        .expect("recovery preserved");
    assert_eq!(persisted_recovery.source_tx_hash, recovery.source_tx_hash);
    assert_eq!(persisted_recovery.status, recovery.status);
    assert_eq!(persisted_recovery.attempt_count, recovery.attempt_count);
    assert!(handle.utxos.read().await.is_empty());
    assert_eq!(
        handle.observation().ppoi_workflow_status(),
        &workflow_before
...
#[tokio::test]
async fn authorized_external_verification_projection_failure_does_not_commit() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let cache_store = RecordingCacheStore::new(Arc::clone(&store));
    let list_key = FixedBytes::from([0xaf; 32]);
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-external-verify-projection-failure");
    let mut pending =
        external_pending_output_record(&cfg, 0xae, list_key, PendingOutputPoiRole::BroadcasterFee);
    pending.submitted_poi_list_keys = vec![list_key];
    store
        .put_pending_output_poi_context(&pending)
        .expect("store external pending context");
    ...
    let authority = WalletPrivateMutationAuthority::new(&handle, 0, &cancel);
    cache_store.fail_pending_output_list_call(1);
    let commit_calls_before = cache_store.state().meta_calls;
    ...
    assert_eq!(outcome.completed, 0);
    assert_eq!(outcome.errors, 1);
    assert_eq!(cache_store.state().meta_calls, commit_calls_before);
    assert!(
        store
            .get_pending_output_poi_context(
                cfg.chain.chain_id,
...
async fn force_resubmit_preflight_skips_spent_output_before_remote_submit() {
    ...
    wallet_utxo
        ...
        .insert(list_key, PoiStatus::Missing);
    let mut pending = matching_pending_output_record(&cfg, &wallet_utxo, list_key);
    pending.output_role = PendingOutputPoiRole::Change;
    pending.submitted_poi_list_keys = vec![list_key];
    store
        .put_pending_output_poi_context(&pending)
        .expect("store pending context");
...
async fn multi_proof_submit_revalidates_between_remote_calls() {
    ...
    let subject = PendingOutputPoiSubject::Owned(ExpectedWalletOutput::new(&wallet_utxo));
...
async fn output_recovery_mixed_statuses_request_and_persist_only_recoverable_lists() {
        ...
        .enumerate()
    {
        ...
        let mut unrelated_context = external_pending_output_record(
            &cfg,
            0xb0 + case as u8,
            unrelated_list_key,
            PendingOutputPoiRole::BroadcasterFee,
        );
        unrelated_context.submitted_poi_list_keys = vec![unrelated_list_key];
        store
            .put_pending_output_poi_context(&unrelated_context)
            .expect("store unrelated pending context");
        let outcome = apply_owned_poi_private_delta_on_actor(
            &handle,
            &cancel,
            0,
        ...
        assert_eq!(
            ...
            "target-list recovery must retain unrelated active-list workflow"
        );
        let persisted = store
            .get_pending_output_poi_context(
                cfg.chain.chain_id,
...
async fn assert_valid_output_recovery_recovers_new_active_list(force_retry: bool) {
    ...
    let outcome = apply_owned_poi_private_delta_on_actor(
        ...
        OwnedPoiPrivateDelta::OutputRecovery {
            ...
            active_list_keys: active_list_keys.to_vec(),
            target_list_keys: recoverable_list_keys,
            required_poi_status: ExpectedPoiStatus::Recoverable,
            pending_update: Box::new(Some((ExpectedRecordState::Absent, pending.clone()))),
            expected_recovery,
            action: OutputPoiRecoveryAction::Detected {
...
async fn successful_submission_intent_survives_timestamp_only_poi_refresh() {
    ...
    let outcome = apply_owned_poi_private_delta_on_actor(
        ...
        OwnedPoiPrivateDelta::PendingSubmission {
            subject: PendingOutputPoiSubject::Owned(expected_output),
...
#[tokio::test]
async fn submission_workflow_projection_failure_aborts_before_commit_and_retry_converges() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let cache_store = RecordingCacheStore::new(Arc::clone(&store));
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-submission-workflow-projection-failure");
    ...
    handle
        ...
        .publish_ppoi_workflow_status(workflow_before);
    let cancel = CancellationToken::new();
    let submission_delta = || OwnedPoiPrivateDelta::PendingSubmission {
        subject: PendingOutputPoiSubject::Owned(expected_output.clone()),
    ...
    cache_store.fail_pending_output_list_call(0);
    let commit_calls_before = cache_store.state().meta_calls;

    let failure = apply_owned_poi_private_delta_on_actor(
        &handle,
        &cancel,
        0,
        store.as_ref(),
    ...
    assert!(matches!(failure, Err(WalletCacheError::Crypto)));
    assert_eq!(cache_store.state().meta_calls, commit_calls_before);
    let retained = store
        .get_pending_output_poi_context(
            cfg.chain.chain_id,
        ...
        .expect("load retained pending context")
        .expect("pending context retained");
    assert!(retained.submitted_poi_list_keys.is_empty());
    assert_eq!(
        handle.observation().ppoi_workflow_status(),
        &workflow_before
    );

    let applied = apply_owned_poi_private_delta_on_actor(
        &handle,
        &cancel,
        0,
        store.as_ref(),
    ...
    .await
    .expect("retry submission after transient projection failure");
    ...
    assert_eq!(cache_store.state().meta_calls, commit_calls_before + 1);
    assert_eq!(
        handle.observation().ppoi_workflow_status(),
        &WalletPpoiWorkflowStatus {
            awaiting_validation: 1,
...
#[tokio::test]
async fn owned_status_refresh_validation_rejects_validity_downgrade_before_apply() {
    let root_dir = temp_db_root();
    let store = DbStore::open(DbConfig {
        root_dir: root_dir.clone(),
    })
    .expect("open db");
    let mut cfg = wallet_config(U256::ZERO);
    cfg.cache_key = test_cache_key("wallet-owned-valid-downgrade");
    ...
    let outcome = apply_owned_poi_private_delta_on_actor(
        ...
        OwnedPoiPrivateDelta::VerifiedValid {
            subject: PendingOutputPoiSubject::Owned(expected_output),
            evidence: PendingOutputPoiValidationEvidence::OwnedStatusRefresh,
            expected_context_fingerprint,
            expected_recovery: ExpectedRecordState::Absent,
            expected_poi_list_state: Some(expected_poi_list_state),
            active_list_keys: vec![list_key],
    ...
    .await
    .expect("apply stale owned-status validation");
...
async fn stale_submission_behind_verified_valid_cannot_resurrect_or_downgrade() {
    ...
    let verified = OwnedPoiPrivateDelta::VerifiedValid {
        subject: PendingOutputPoiSubject::Owned(expected_output),
        evidence: PendingOutputPoiValidationEvidence::SubmittedStatus,
        expected_context_fingerprint,
        expected_recovery: super::expected_recovery_state(Some(&recovery))
            .expect("recovery fingerprint"),
        expected_poi_list_state: Some(expected_poi_list_state),
...
async fn verified_valid_rejects_changed_context_or_shield_blocked_but_accepts_recoverable() {
    ...
    let stale_authority_outcome = apply_owned_poi_private_delta_on_actor(
        &handle,
        &cancel,
        1,
        &store,
        ...
        OwnedPoiPrivateDelta::VerifiedValid {
            ...
            expected_context_fingerprint: expected_context_fingerprint.clone(),
            expected_recovery: ExpectedRecordState::Absent,
            expected_poi_list_state: Some(expected_poi_list_state.clone()),
            active_list_keys: vec![list_key],
            valid_list_keys: vec![list_key],
            now: 19,
        },
    ...
    .await;
    assert!(matches!(
        stale_authority_outcome,
        Err(WalletCacheError::Crypto)
    ));
    assert_eq!(
        handle.observation().ppoi_workflow_status(),
        &workflow_before
...
#[tokio::test]
async fn sender_materialization_atomically_installs_standard_records_without_balance_mutation() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let mut cfg = wallet_config(U256::from(42));
    cfg.cache_key = test_cache_key("sender-materialization");
    let list_key = FixedBytes::from([0xa1; 32]);
    let candidate_source = source(0xa2);
    let mut input = test_wallet_utxo(7);
    input.spent = Some(candidate_source.clone());
    ...
    pending.txid_merkleroot_index = Some(77);
    pending.source_operation_id = Some("test-recovered-sender-inner-7".to_string());
    pending.output_commitment = output.poi.commitment;
    pending.output_npk = output.poi.npk;
    pending.observation = Some(local_db::PendingOutputPoiObservation {
        output_tree: u64::from(output.tree),
        output_position: output.position,
        tx_hash: candidate_source.tx_hash,
    ...
    let (public_data_plane, public_data_fence) = sender_candidate_public_data_fence(
        &store,
        &cfg,
        &candidate,
        &input,
    ...
    let mut handle = test_wallet_handle(vec![input.clone()]);
    handle.cache_key = cfg.cache_key.clone();
    let before = wallet_utxo_stable_identity(&input);
    let cancel = CancellationToken::new();
    let delta = || OwnedPoiPrivateDelta::SenderCandidateMaterialization {
        expected_candidate: candidate.clone(),
        public_data_fence: public_data_fence.clone(),
        active_list_keys: vec![list_key],
    ...
    assert!(
        ...
        .await
        .is_err(),
        "reset-generation stale work must not enter private apply"
    );
    let mut retired_handle = test_wallet_handle(vec![input.clone()]);
    retired_handle.cache_key = cfg.cache_key.clone();
    retired_handle.retire_actor();
    assert!(
        apply_owned_poi_private_delta_on_actor(
            &retired_handle,
            &cancel,
        ...
        .await
        .is_err(),
        "replaced actor work must not enter private apply"
    );
    handle.utxos.write().await[0].spent = None;
    assert_eq!(
        apply_owned_poi_private_delta_on_actor(
            &handle,
            &cancel,
        ...
        .await
        .expect("reject changed spent input"),
        PoiPrivateApplyOutcome::Skipped
    );
    handle.utxos.write().await[0].spent = input.spent.clone();

    let mut replacement_source = candidate.source.clone();
    replacement_source.block_timestamp += 1;
    let replacement =
        sender_candidate_for_spent_input(cfg.cache_key.clone(), &input, replacement_source);
    store
        .put_opaque_wallet_private_row(
            &replacement.namespace(),
            WalletPrivateRecordKind::SenderTransactionCandidate,
            &OpaqueWalletPrivateRow {
                row_id: replacement.row_identity(),
                payload: replacement.encode().expect("encode replacement candidate"),
            },
        )
        .expect("replace sender candidate");
    assert_eq!(
        apply_owned_poi_private_delta_on_actor(
            &handle,
            &cancel,
    ...
    store
        .put_opaque_wallet_private_row(
            ...
            &OpaqueWalletPrivateRow {
                row_id: candidate.row_identity(),
                payload: candidate.encode().expect("restore candidate"),
            },
        )
        .expect("restore sender candidate");
    ...
    assert_eq!(
        ...
        .await
        .expect("reject stale public epoch"),
        PoiPrivateApplyOutcome::Skipped
    );

    let (current_public_data_plane, current_public_data_fence) =
        sender_candidate_public_data_fence(
            &store,
            &cfg,
            &candidate,
            &input,
    ...
    row_reset_plane
        .reset_public_cache()
        .await
        .expect("replace persisted public rows without changing captured epoch");
    let (_replacement_plane, _) = sender_candidate_public_data_fence(
        &store,
        &cfg,
        &candidate,
        &input,
    ...
    .await;
    let row_delta = || OwnedPoiPrivateDelta::SenderCandidateMaterialization {
        expected_candidate: candidate.clone(),
        public_data_fence: current_public_data_fence.clone(),
        active_list_keys: vec![list_key],
        pending_updates: vec![pending.clone()],
        recovery_updates: vec![recovery.clone()],
        owned_substitutes: Vec::new(),
    ...
    assert_eq!(
        ...
        .await
        .expect("reject replaced public TXID row"),
        PoiPrivateApplyOutcome::Skipped
    );

    let (_final_public_data_plane, final_public_data_fence) = sender_candidate_public_data_fence(
        &store,
        &cfg,
        &candidate,
        &input,
    ...
    assert!(
        store
            ...
            .expect("load consumed candidate")
            .is_none()
    );
    assert!(
        store
            ...
            .expect("load pending sibling")
            .is_some()
    );
    assert!(
        store
            .get_output_poi_recovery(cfg.chain.chain_id, &cfg.cache_key, &output.poi.commitment,)
            .expect("load recovery sibling")
            .is_some()
    );
    let after = handle.utxos.read().await;
    assert_eq!(after.len(), 1);
    assert_eq!(wallet_utxo_stable_identity(&after[0]), before);
    assert_eq!(after[0].spent, input.spent);
    drop(after);
...
#[tokio::test]
async fn sender_materialization_consumes_exact_covered_and_mixed_candidates_without_overwrite() {
    for covered_count in [1_usize, 2] {
        let root_dir = temp_db_root();
        let store = Arc::new(
            DbStore::open(DbConfig {
                root_dir: root_dir.clone(),
        ...
        let fixture = sender_materialization_test_fixture(
            &store,
            &format!("sender-materialization-covered-{covered_count}"),
            2,
        )
        .await;
        for index in 0..covered_count {
            let mut context = fixture.pending[index].clone();
            context.submitted_poi_list_keys = context.required_poi_list_keys.clone();
            context.terminal_error = Some("preserved progress".to_string());
            let mut recovery = fixture.recoveries[index].clone();
            recovery.status = OutputPoiRecoveryStatus::Submitted;
            recovery.attempt_count = 4;
            recovery.last_error = Some("preserved recovery progress".to_string());
            store
                .put_pending_output_poi_context(&context)
                .expect("seed exact covered context");
            store
                .put_output_poi_recovery(&recovery)
                .expect("seed exact covered recovery");
        }
        let covered_before = (0..covered_count)
            .map(|index| {
                (
                    rmp_serde::to_vec(
                        &store
                            .get_pending_output_poi_context(
                                ...
                                &fixture.pending[index].output_commitment,
                            )
                            .expect("load covered context")
                            .expect("covered context present"),
                    )
                    .expect("encode covered context"),
                    rmp_serde::to_vec(
                        &store
                            .get_output_poi_recovery(
                                fixture.cfg.chain.chain_id,
                            ...
                            .expect("load covered recovery")
                            .expect("covered recovery present"),
                    )
                    .expect("encode covered recovery"),
                )
            })
            .collect::<Vec<_>>();
        ...
        assert_eq!(
            apply_owned_poi_private_delta_on_actor(
                ...
                OwnedPoiPrivateDelta::SenderCandidateMaterialization {
                    ...
                    active_list_keys: fixture.pending[0].required_poi_list_keys.clone(),
            ...
            .await
            .expect("consume covered candidate"),
            PoiPrivateApplyOutcome::Applied {
                utxo_changed: false
            },
        ...
        assert_eq!(
            ...
            .await
            .expect("replay consumed candidate"),
            PoiPrivateApplyOutcome::Skipped
        );
        for (index, before) in covered_before.iter().enumerate() {
            assert_eq!(
                rmp_serde::to_vec(
                    &store
                        .get_pending_output_poi_context(
                        ...
                        .expect("reload covered context")
                        .expect("covered context retained"),
                )
                .expect("encode retained context"),
                before.0
            );
            assert_eq!(
                rmp_serde::to_vec(
                    &store
                        ...
                        .expect("reload covered recovery")
                        .expect("covered recovery retained"),
                )
                .expect("encode retained recovery"),
                before.1
            );
        }
        for index in covered_count..fixture.pending.len() {
            assert!(
                store
                    .get_pending_output_poi_context(
                        fixture.cfg.chain.chain_id,
                    ...
                    .expect("load absent materialized context")
                    .is_some()
            );
            assert!(
                store
                    ...
                    .expect("load absent materialized recovery")
                    .is_some()
            );
        }
...
#[tokio::test]
async fn sender_materialization_persists_only_external_output_and_requires_exact_owned_substitute()
{
    for case in ["exact", "missing", "duplicate", "changed", "spent"] {
        let root_dir = temp_db_root();
        let store = Arc::new(
            DbStore::open(DbConfig {
                root_dir: root_dir.clone(),
        ...
        let fixture = sender_materialization_test_fixture(
            &store,
            &format!("sender-materialization-owned-{case}"),
            2,
        )
        .await;
        let owned_candidate_output = &fixture.candidate.outputs[1];
        let mut owned = WalletUtxo::new(Utxo::new(
            owned_candidate_output
                .note
                .clone()
                .expect("owned candidate note"),
            owned_candidate_output.tree,
            owned_candidate_output.position,
            fixture.candidate.source.clone(),
            UtxoCommitmentKind::Transact,
        ));
        let expected_owned = ExpectedWalletOutput::new(&owned);
        match case {
            "changed" => owned.utxo.poi.npk = FixedBytes::from([0xee; 32]),
            "spent" => owned.spent = Some(source(0xef)),
            _ => {}
        }
        let external = fixture.pending[0].clone();
        let proof_outputs = vec![
            pending_output_poi_submit_identity(
                &external,
                external.observation.as_ref().expect("external observation"),
            )
            .expect("external submit identity")
            .derived_blinded_commitment,
            expected_owned.blinded_commitment(),
        ];
        let mut wallet_outputs = vec![fixture.input.clone()];
        if case != "missing" {
            wallet_outputs.push(owned.clone());
        }
        if case == "duplicate" {
            wallet_outputs.push(owned);
        }
        let mut handle = test_wallet_handle(wallet_outputs);
        handle.cache_key = fixture.cfg.cache_key.clone();
        let outcome = apply_owned_poi_private_delta_on_actor(
            &handle,
            ...
            OwnedPoiPrivateDelta::SenderCandidateMaterialization {
                expected_candidate: fixture.candidate.clone(),
                public_data_fence: fixture.fence,
                active_list_keys: external.required_poi_list_keys.clone(),
                pending_updates: vec![external.clone()],
        ...
        .await
        .expect("materialization apply result");
        ...
        assert_eq!(
            store
                .get_pending_output_poi_context(
                    fixture.cfg.chain.chain_id,
                    &fixture.cfg.cache_key,
                    &fixture.pending[0].output_commitment,
                )
                .expect("load external context")
                .is_some(),
            case == "exact"
        ...
        assert!(
            store
                .get_pending_output_poi_context(
                    fixture.cfg.chain.chain_id,
                    &fixture.cfg.cache_key,
                    &fixture.pending[1].output_commitment,
                )
                .expect("load owned context")
                .is_none()
        );
...
#[tokio::test]
async fn public_reset_waits_for_sender_materialization_commit_guard_without_deadlock() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let public_data_plane =
        ChainPublicDataPlane::new(Arc::clone(&store), Arc::new(AtomicU64::new(0)));
    let commit_guard = public_data_plane.acquire_commit_guard().await;
    let reset_plane = public_data_plane.clone();
    let (started_tx, started_rx) = oneshot::channel();
    let reset = tokio::spawn(async move {
        let _ = started_tx.send(());
        reset_plane.reset_public_cache().await
    });
    started_rx.await.expect("reset task started");
    tokio::task::yield_now().await;
    assert!(!reset.is_finished());
    assert_eq!(public_data_plane.current_epoch().value, 0);

    drop(commit_guard);
    let reset = reset
        .await
        .expect("reset task completed")
        .expect("public reset succeeded");
    assert_eq!(reset.previous_epoch.value, 0);
    assert_eq!(reset.new_epoch.value, 1);
...
#[tokio::test]
async fn sender_materialization_partial_or_conflicting_pair_retains_everything() {
    for case in ["partial", "conflicting"] {
        ...
        let fixture = sender_materialization_test_fixture(
            &store,
            &format!("sender-materialization-{case}"),
            2,
        )
        .await;
        let mut existing = fixture.pending[0].clone();
        if case == "conflicting" {
            existing.railgun_txid += U256::from(1);
        }
        store
            .put_pending_output_poi_context(&existing)
            .expect("seed predecessor context");
        if case == "conflicting" {
            store
                .put_output_poi_recovery(&fixture.recoveries[0])
                .expect("seed conflicting pair recovery");
        }
        let mut handle = test_wallet_handle(vec![fixture.input.clone()]);
        handle.cache_key = fixture.cfg.cache_key.clone();
        let cancel = CancellationToken::new();
        ...
        assert_eq!(
            ...
            .await
            .expect("reject nonstandard predecessor"),
            PoiPrivateApplyOutcome::Skipped,
            "case={case}"
        );
        assert!(
        ...
        assert!(
            store
                ...
                .expect("load untouched absent context")
                .is_none(),
            "case={case}"
        );
        assert!(
            store
                ...
                .expect("load untouched absent recovery")
                .is_none(),
            "case={case}"
        );
...
#[tokio::test]
async fn sender_materialization_persistence_failure_retains_candidate_without_derived_rows() {
    let root_dir = temp_db_root();
    let store = Arc::new(
        DbStore::open(DbConfig {
            root_dir: root_dir.clone(),
    ...
    let fixture =
        sender_materialization_test_fixture(&store, "sender-materialization-failure", 2).await;
    let cache_store = RecordingCacheStore::new(Arc::clone(&store));
    cache_store.fail_next_commit();
    let mut handle = test_wallet_handle(vec![fixture.input.clone()]);
    handle.cache_key = fixture.cfg.cache_key.clone();
    ...
    for context in &fixture.pending {
        assert!(
            store
                .get_pending_output_poi_context(
                    fixture.cfg.chain.chain_id,
